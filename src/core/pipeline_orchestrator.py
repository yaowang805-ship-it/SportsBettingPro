...
class PipelineOrchestrator:
    ...
    def _startup_integrity_check(self):
        ...
        # 4) Pinnacle API 连通性检查 (2026-09-18 暂停 Pin 轮询, 跳过此检查+告警)
        from config.settings import PIN_POLLING_PAUSED
        if PIN_POLLING_PAUSED:
            logger.info("[自检] Pin 轮询已暂停, 跳过 Pinnacle API 连通性检查")
        else:
    ...
    def _tick(self):
        ...
        if self._is_in_scan_window(now) and not _full_scan_running:
            ...
            # 2026-09-15 早盘恢复频次(已有早盘释放盘口 ht/ht_dc/1x2, 见 CLV>2% 判据): far 24-72h 15min→10min,
            # 回退 9-14 的降频(当时早盘无释放)。10min 是降频前的已知安全值, 不增 Pin 压力(瞬时速率仍 12req/s 内)。
