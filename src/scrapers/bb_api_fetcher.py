...
MARKET_TYPES = {
    ...
    13: {  # 排球
        ...
        "ht_ml": 13006,  # 第1盘盘胜者(Set Winner-Set 1); 全场独赢13001 在 pe=13002 下不存在
    },
}
...
SPORT_PERIODS = {
    ...
    18: {"ft": 18001},
    47: {"ft": 47001},
    2: {"ft": 2001, "ht": 2003},  # 冰球第1节 pe=2003 (Period 1)
    13: {"ft": 13001, "ht": 13002},  # 排球第1盘 pe=13002 (Set 1)
}
...
# ⚠️ 2026-09-13 补 6 运动(冰球/乒乓/拳击/MMA/羽毛球/排球)的 period 码, 使它们也能提取比分结算。
#    这些码取自 SPORT_PERIODS 的 ft, 属推断(无实盘订单验证)。注意 nsg 的 score pe 可能与
#    mg 的 ft 差 1(足球 1000 vs ft=1001、网球 5000 vs ft=5001), 接这 6 运动实盘前必须
#    实测确认 nsg 里真正的全场比分 pe + tyg=5 是否命中, 否则会拿错比分结算(见 [[settle-reliability-whitelist-20260912]])。
SCORE_PE_BY_SID = {
    1: 1000,   # 足球 全场
    3: 3001,   # 篮球 全场
    ...
    2: 2001,   # 冰球 全场 (推断, 需验证)
    15: 15001,  # 乒乓球 全场 (推断, 需验证)
    19: 19001,  # 拳击 全场 (推断, 需验证)
    18: 18001,  # MMA 全场 (推断, 需验证)
    47: 47001,  # 羽毛球 全场 (推断, 需验证)
    13: 13001,  # 排球 全场 (推断, 需验证)
}
...
def _merge_single_match(platform_matches):
    ...
    for platform, m in platform_matches[1:]:
        ...
        if base_hc and plat_hc and isinstance(base_hc, dict) and isinstance(plat_hc, dict):
            # 让球线 0.0(平手盘)是合法值, 不能用 `or` —— 0.0 是 falsy, 会把 0 线误判成 None,
            # 进而跳过线对齐检查, 把 BB 的 0 线和 FB 的 -0/0.5 线赔率混并 → 假+EV。
