...
# 滚球实盘(2026-09-09 用户要求: 覆盖所有运动+所有盘口, 取消"只投小球"限制)。滚动预算(结算后释放额度)。
...
# EV-Kelly 最优定仓, 单注上限 ¥250(1/4预算)兼顾分散; 所有盘口(大球/小球/1x2/让球)都可下真单。
LIVE_REAL_BET_ENABLED = True
LIVE_UNDER_MAX_STAKE = 250  # 单注上限(EV-Kelly 定仓, 上限1/4预算)
