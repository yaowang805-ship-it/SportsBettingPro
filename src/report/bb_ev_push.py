...
KELLY_BY_MARKET = {
    ...
    "ht_dc": 0.40, # 2026-08-28: 实盘-17.3%, CLV高估21pp, 回退(不再加仓)
    "corner": 0.45, # V5.1: 5.7万 AH数据, 第4重要市场
    "btts": 0.45,  # V5.2: 推导自OU, 无独立Pinnacle数据 → 下调
    "dnb":  0.45,  # V5.2: 推导 → 下调
    "dc":   0.40,  # V5.2: 推导 → 下调
    "ht":   0.50,  # 2026-08-27: 半场1X2 正 edge(+3.6%/71%), 提到半凯利
...
def _calc_kelly_stakes(opps: list) -> list:
    ...
    # 投注额分层(2026-09-09 用户要求): 按 edge 档(强/中/弱/假)固定金额。
    # 2026-09-10 减半止血: 早盘 CLV 全负(假edge), 观察库纸面盈利是假的(实盘全亏), 找到根因前先缩小。
    for o in opps:
        _w = o.get("_kelly_weight", 0)
        if _w > 0:
            _mult = o.get("_edge_mult", MEDIUM_EDGE_MULT)
            if _mult >= 15.0:
                _stake = 190      # 强 edge(减半)
            elif _mult >= 10.0:
                _stake = 125      # 中 edge
            elif _mult >= 5.0:
                _stake = 65       # 弱 edge
            else:
                _stake = 30       # 假 edge(不低于30)
        else:
            _stake = 0
        o["_raw_stake"] = _stake
