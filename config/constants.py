...
# ── Kelly 计算 ──
# 2026-09-03 用户要求提到半凯利(0.50): 结算链路已闭环(BB赛果百分百覆盖) + CLV采集修复后,
# 真实edge可信任, 恢复 0.50 提升资金效率(原 08-29 降到 0.33 抗回撤)。
KELLY_FRACTION = 0.50              # Kelly 分数（0.33 → 0.50, 用户 2026-09-03 要求提到半凯利）
...
def get_tier_strategy(sport: str, league: str, tier: int = None) -> dict:
    ...
    # ── 运动差异化 ──
    # V5.2: ev_floor 数据化 (Pinnacle margin 1X2~2-3%/OU~4-7%), 旧的 T3=4%/T4=6% 过度保守
    if sport == "football":
        if tier == 1:
