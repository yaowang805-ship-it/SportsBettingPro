...
# =====================================================================
FB_1X2_WEIGHTS = {
    # league_keyword → [<2.0, 2.0-3.0, 3.0-4.0, 4.0-5.0, 5.0-10.0, >10.0]
    # 数据源: Pinnacle收盘39K场 + 我们112笔结算
    # 3.0-4.0和4.0-5.0分开: 前者ROI-75%, 后者+22.6%
    "德甲": [4, 6, 2, 4, 2, 2],
    "德乙": [2, 2, 1, 4, 0, 0],
    "英超": [4, 6, 1, 2, 2, 2],
    "英冠": [4, 4, 1, 2, 3, 0],
    ...
    "苏超": [4, 4, 1, 2, 0, 0],
    "希超": [2, 2, 1, 2, 0, 0],
    "_DEFAULT": [3, 3, 0, 2, 1, 0],  # 3-4跳, 4-5给2%, 5-10给1%试探
}
...
# =====================================================================
FB_OU_WEIGHTS = {
    # [<2.0, 2.0-3.0, 3.0-4.0, 4.0-5.0, 5.0-10.0, >10.0]
    "德甲": [4, 2, 1, 4, 0, 0],
    "德乙": [4, 2, 1, 0, 0, 0],
    "英超": [2, 6, 1, 4, 0, 0],
    "英冠": [4, 2, 1, 0, 0, 0],
...
def _get_odds_index(odds: float, sport: str = "football") -> int:
    ...
    if sport in ("tennis",):
        ...
        # 足球/篮球/棒球等: 6区间 (3.0-4.0和4.0-5.0结算ROI差异大, 必须分开)
        if odds < 2.0: return 0
        elif odds < 3.0: return 1
        elif odds < 4.0: return 2   # 3.0-4.0 (结算 -75%, 严格限制)
        elif odds <= 5.0: return 3  # 4.0-5.0 (结算 +22.6%, 保留!)
        elif odds <= 10.0: return 4
        else: return 5
